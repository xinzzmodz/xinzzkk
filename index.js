...
async function startclient() {
    const client = clientConnect({
        ...
        browser: ['Criado por Risky','Safari','1.0.0'],
        auth: state
    })
    ...
    let halal = await client.sendContact(callerId, global.owner)
    client.sendMessage(callerId, { text: `*Automatic block system!*\n*Don't call bot*!\n*Please contact the owner to open !*`}, { quoted : halal })
    await sleep(8000)
    ...
    client.ev.on('messages.upsert', async chatUpdate => {
        ...
        require("./malia")(client, m, chatUpdate, store)
        } catch (err) {
            console.log(err)
        }
    })
    ...
    client.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                //welcome\\
        let nama = await client.getName(num)
...
└┬❖ 「 @${xeonName.split("@")[0]}  」
   ...
   └───────────────┈ ⳹`
let buttons = [
{buttonId: `wkwwk`, buttonText: {displayText: 'Obrigado 💐'}, type: 1}
]
let buttonMessage = {
...
title: `${ownername}`,
body: `Leia a descrição do grupo!`,
mediaType:2,
thumbnail: XeonWlcm,
sourceUrl: `${websitex}`,
mediaUrl: `${websitex}`
...
client.sendMessage(anu.id, buttonMessage, {quoted:unicorndoc})
                } else if (anu.action == 'remove') {
                	const xeonbuffer = await getBuffer(ppuser)
                    const xeontime = moment.tz('America/Fortaleza').format('HH:mm:ss')
	                const xeondate = moment.tz('America/Fortaleza').format('DD/MM/YYYY')
                	let xeonName = num
